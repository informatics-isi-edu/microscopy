...
#
"""
Load configuration for deleting a video from YouTube.
Check with ermrest for videos that need to be deleted from YouTube.
Update the Delete_Youtube table with the deletion result.
"""
...
from rbk_delete_youtube_lib.client import YouTubeClient
...
def load(config_filename):
    ...
    if url == None:
        ...
        logger.error('Launch the script "env URL=https://foo.org/ermrest/catalog/N rbk_delete_youtube.py --config config-file".')
        return None
...
try:
    ...
    sys.stderr.write('\nusage: env URL=https://foo.org/ermrest/catalog/N rbk_delete_youtube.py --config config-file\n\n')
    sys.exit(1)
